...
class ChatService:
    async def process_message(self, wa_id: str, name: str, message_body: str):
        ...
        async for session in get_session():
            try:
                ...
                ai_response = await openai_service.generate_response(message_body, history)
                
                # 6. Save Assistant Message
                await self._save_message(session, conversation.id, "assistant", ai_response)
    ...
    async def _get_or_create_user(self, session: AsyncSession, wa_id: str, name: str) -> User:
    ...
    async def _get_or_create_conversation(self, session: AsyncSession, user_id: int) -> Conversation:
    ...
    async def _save_message(self, session: AsyncSession, conversation_id: int, role: str, content: str):
        message = Message(conversation_id=conversation_id, role=role, content=content)
        session.add(message)
        await session.commit()
    
    async def _get_conversation_history(self, session: AsyncSession, conversation_id: int, limit: int = 10) -> list[dict]:
